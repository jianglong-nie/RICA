...
public:
  ...
  ~ATPTxItem()
  {
    ...
    bool m_toAggregator;        //!< 是否计入 ACW 并尝试在交换机聚合
};
...
class ATPTxBuffer : public Object
{
  public:
     ...
     */
    Ptr<Packet> SendPacket(bool toAggregator);

    /**
     * \brief 获取计入ACW的飞行包数
     * \return 聚合器窗口内飞行包数
     */
    uint32_t GetAggregatorInFlight() const;
    ...
    typedef std::deque<ATPTxItem*> PacketQueue; //!< 数据包队列类型
