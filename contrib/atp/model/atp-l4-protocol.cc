...
ATPL4Protocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::ATPL4Protocol")
            ...
            .AddAttribute("MaxAggregators",
                         "Maximum number of aggregator slots.",
                         UintegerValue(ATPL4Protocol::DEFAULT_MAX_AGGREGATORS),
                         MakeUintegerAccessor(&ATPL4Protocol::SetMaxAggregators,
                                              &ATPL4Protocol::GetMaxAggregators),
    ...
    : m_endPoints(new Ipv4EndPointDemux())
{
    ...
    m_aggregators.resize(m_maxAggregators);
}
...
void
ATPL4Protocol::SetEnableHierarchicalAggregation(bool enable)
{
    NS_LOG_FUNCTION(this << enable);
    m_enableHierarchicalAggregation = enable;
}

bool
ATPL4Protocol::GetEnableHierarchicalAggregation() const
{
    return m_enableHierarchicalAggregation;
}

void
ATPL4Protocol::SetMaxAggregators(uint32_t maxAggregators)
{
    NS_LOG_FUNCTION(this << maxAggregators);
    m_maxAggregators = maxAggregators;
    m_aggregators.clear();
    m_aggregators.resize(m_maxAggregators);
    m_aggCount.clear();
}
...
uint32_t
ATPL4Protocol::GetMaxAggregators() const
{
    return m_maxAggregators;
}
...
ATPL4Protocol::FilterPacket(Ptr<Packet> packet)
{
    ...
    if (atpTag.m_isAck == 1) {
        std::size_t index = Aggregator::HashToIndex(atpTag.GetJobId(), atpTag.GetSeqNum(), m_maxAggregators);
