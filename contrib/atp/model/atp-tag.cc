...
void
ATPTag::SetProg(uint32_t progress)
{
    NS_LOG_FUNCTION(this << progress);
    m_progress = progress;
}

uint32_t
ATPTag::GetProg() const
{
    NS_LOG_FUNCTION(this);
    return m_progress;
}
...
{
    NS_LOG_FUNCTION(this);
    return 1 + 1 + 4 + 4 + 1 + 4 + 1 + 1 + 1 + 1 + 1 + 1 + 1 + 2 + 2 + 2 + 8 + 4;
    // faninDegree0 + faninDegree1 + bitmap0 + bitmap1 + jobId + seqNum + aecn
    // + ecn + isAck + overflow + resend + collision + edgeSwitchIdentifier + size + sourcePort + destinationPort + m_sendTimestamp + m_progress
}
