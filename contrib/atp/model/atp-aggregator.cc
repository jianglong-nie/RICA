...
JobAggregator::ProcessPacket(Ptr<Packet> packet, uint32_t incoming_flat_bitmap)
{
    ...
    ATPTag atpTag;
    packet->PeekPacketTag(atpTag);
    m_collision |= atpTag.m_collision;
    m_ecn |= atpTag.m_ecn;
    
    // 存储第一个到达的数据包用于交付
    if (!m_packet) {
        m_packet = packet->Copy();
        m_firstArrivalTime = Simulator::Now();
    }
