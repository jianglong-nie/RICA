...
ATPSocket::SendWindowData()
{
    ...
    uint32_t acw = m_txBuffer->GetACW(); // 聚合器窗口
    uint32_t lcw = m_txBuffer->GetLCW(); // 链路窗口
    uint32_t pendingId = m_txBuffer->GetPendingFrontPacketId();

    NS_LOG_INFO("SendWindowData - Time: " << Simulator::Now().GetSeconds()
                                          << " leftBound: " << leftBound << " ACW: " << acw
                                          << " LCW: " << lcw << " pendingFrontId: " << pendingId);

    // 当lcw为1时的特殊处理
    if (lcw == 1)
    {
        NS_LOG_INFO("Special handling for lcw=1");
        // 确保在窗口范围内发送，并且不会过于激进
        if (pendingId <= leftBound + lcw)
        {
            Ptr<Packet> packet = m_txBuffer->SendPacket();
            if (packet == nullptr)
            {
                NS_LOG_INFO("No packet to send in lcw=1 case");
                return;
            }
            NS_LOG_INFO("Sending packet in lcw=1 case");
            DoSend(packet);
    ...
    while (m_txBuffer->GetPendingFrontPacketId() <= leftBound + lcw)
    {
        Ptr<Packet> packet = m_txBuffer->SendPacket();
        if (packet == nullptr)
        {
        ...
        NS_LOG_INFO("Sending packet to aggregator, seq=" << packet->GetUid());
        DoSend(packet);
    }
}
...
ATPSocket::DoSend(Ptr<Packet> p)
{
    ...
    if (p->PeekPacketTag(atpTag))
    {
        ...
        m_bitmap0 = atpTag.GetBitMap0();
        m_bitmap1 = atpTag.GetBitMap1();
        m_jobId = atpTag.GetJobId();
...
ATPSocket::ReceiveAck(ATPTag atpTag)
{
    ...
    if (m_txBuffer->IsOrderedAck(atpTag.GetSeqNum()))
    {
        ...
        bool isCollided = (atpTag.m_collision == 1);

        if (!isEcn)
        {
            if (!isCollided)
                m_txBuffer->CountCongestion(0);
            else
                m_txBuffer->CountCongestion(2);
        }
        else
        {
            if (!isCollided)
                m_txBuffer->CountCongestion(1);
            else
                m_txBuffer->CountCongestion(3);
        }
        // RTT估计
        uint64_t sendTime = atpTag.GetSendTimestamp();
        Time rttUs = Simulator::Now() - MicroSeconds(sendTime);

        uint32_t stragglerBitmap0 = atpTag.m_lastbitmap0;
        uint32_t stragglerBitmap1 = atpTag.m_lastbitmap1;
        uint8_t jobId = atpTag.GetJobId();
        bool isStraggler = (m_jobId == jobId) && (m_bitmap0 == stragglerBitmap0) &&
                           (m_bitmap1 == stragglerBitmap1);

        uint64_t waitingTime = atpTag.GetWaitingTime();

        if (isStraggler)
        {
            // NS_LOG_ERROR(m_bitmap0 << " " << m_bitmap1 << " " << rttUs << " " << atpTag.GetWaitingTime());
        }
        if (m_jobId == jobId)
        {
            m_txBuffer->UpdateStraggler(stragglerBitmap0, stragglerBitmap1, waitingTime, rttUs);
        }

        // 拥塞控制
        m_txBuffer->ProcessCongestion(m_numWorkers, m_bitmap0, m_bitmap1, atpTag.GetSeqNum());
...
ATPSocket::SendAck(ATPTag atpTag, Ipv4Header ipHeader)
{
    ...
    ackTag.m_lastbitmap0 = atpTag.m_lastbitmap0;
    ackTag.m_lastbitmap1 = atpTag.m_lastbitmap1;
    ackTag.SetSendTimestamp(atpTag.GetSendTimestamp());
    ackTag.SetWaitingTime(atpTag.GetWaitingTime());
                           ...
                           Ptr<Ipv4Interface> incomingInterface)
{
    ...
    if (aggregationComplete)
    {
        ...
        ATPTag firstTag;
        aggregatedPacket->PeekPacketTag(firstTag);
        atpTag.SetSendTimestamp(firstTag.GetSendTimestamp());

        atpTag.SetEcn(aggregator.m_ecn);
        atpTag.m_collision = aggregator.m_collision;
        if (atpTag.m_lastbitmap0 == 0 && atpTag.m_lastbitmap1 == 0)
        {
            atpTag.m_lastbitmap0 = atpTag.GetBitMap0();
            atpTag.m_lastbitmap1 = atpTag.GetBitMap1();
        }

        Time waitingTime = Simulator::Now() - aggregator.m_firstArrivalTime;
        atpTag.SetWaitingTime(waitingTime.GetMicroSeconds());
        aggregatedPacket->ReplacePacketTag(atpTag);
