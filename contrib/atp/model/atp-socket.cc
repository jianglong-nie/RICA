...
ATPSocket::SendWindowData()
{
    ...
    if (pendingId > leftBound + 1)
    {
        packetsInFlight = pendingId - leftBound - 1;
    }

    // 计算还可以发送到聚合器的包数（不超过ACW）
    // uint32_t packetsToAggInFlight = std::min(packetsInFlight, acw);
    uint32_t packetsToAggInFlight = std::min(m_txBuffer->GetAggregatorInFlight(), acw);
    uint32_t canSendToAggregator = (acw > packetsToAggInFlight) ? (acw - packetsToAggInFlight) : 0;

    // 先发送到聚合器的包
    uint32_t sentToAggregator = 0;
    while (sentToAggregator < canSendToAggregator &&
           m_txBuffer->GetPendingFrontPacketId() <= leftBound + lcw)
    {
        Ptr<Packet> packet = m_txBuffer->SendPacket(true);
        if (packet == nullptr)
        {
            break;
        }
        ...
        DoSend(packet);
        sentToAggregator++;
        m_txBuffer->m_packetsToAggInPeriod++;
    }
    ...
    {
        Ptr<Packet> packet = m_txBuffer->SendPacket(false);
        if (packet == nullptr)
        {
            break;
        }
        ...
        {
            // 设置collision表示期望不聚合
            atpTag.m_collision = 2;
            packet->ReplacePacketTag(atpTag);
        }

        NS_LOG_INFO("Sending packet to PS directly, seq=" << packet->GetUid());
        DoSend(packet);
        sentToPS++;
    }

    NS_LOG_INFO("Packets in flight: " << packetsInFlight << ". Sent " << sentToAggregator
                                      << " to aggregator, " << sentToPS << " directly to PS");
}
