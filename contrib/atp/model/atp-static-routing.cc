...
NS_LOG_COMPONENT_DEFINE("ATPStaticRouting");

NS_OBJECT_ENSURE_REGISTERED(ATPStaticRouting);

TypeId
ATPStaticRouting::GetTypeId()
{
    static TypeId tid = TypeId("ns3::ATPStaticRouting")
                            .SetParent<Ipv4RoutingProtocol>()
                            .SetGroupName("Internet")
                            .AddConstructor<ATPStaticRouting>();
    return tid;
}

ATPStaticRouting::ATPStaticRouting()
    : m_ipv4(nullptr)
{
...
void
ATPStaticRouting::AddNetworkRouteTo(Ipv4Address network,
...
void
ATPStaticRouting::AddHostRouteTo(Ipv4Address dest,
                                  Ipv4Address nextHop,
                                  uint32_t interface,
                                  uint32_t metric)
{
...
void
ATPStaticRouting::AddHostRouteTo(Ipv4Address dest, uint32_t interface, uint32_t metric)
{
...
void
ATPStaticRouting::SetDefaultRoute(Ipv4Address nextHop, uint32_t interface, uint32_t metric)
{
...
void
ATPStaticRouting::AddMulticastRoute(Ipv4Address origin,
                                     Ipv4Address group,
...
void
ATPStaticRouting::SetDefaultMulticastRoute(uint32_t outputInterface)
{
...
uint32_t
ATPStaticRouting::GetNMulticastRoutes() const
{
    NS_LOG_FUNCTION(this);
...
ATPStaticRouting::GetMulticastRoute(uint32_t index) const
{
...
bool
ATPStaticRouting::RemoveMulticastRoute(Ipv4Address origin,
                                        Ipv4Address group,
                                        uint32_t inputInterface)
{
    NS_LOG_FUNCTION(this << origin << " " << group << " " << inputInterface);
...
void
ATPStaticRouting::RemoveMulticastRoute(uint32_t index)
{
    NS_LOG_FUNCTION(this << index);
    uint32_t tmp = 0;
    for (auto i = m_multicastRoutes.begin(); i != m_multicastRoutes.end(); i++)
...
bool
ATPStaticRouting::LookupRoute(const Ipv4RoutingTableEntry& route, uint32_t metric)
{
...
Ptr<Ipv4Route>
ATPStaticRouting::LookupStatic(Ipv4Address dest, Ptr<NetDevice> oif)
{
...
ATPStaticRouting::LookupStatic(Ipv4Address origin, Ipv4Address group, uint32_t interface)
{
...
uint32_t
ATPStaticRouting::GetNRoutes() const
{
    NS_LOG_FUNCTION(this);
...
ATPStaticRouting::GetDefaultRoute()
{
    NS_LOG_FUNCTION(this);
...
Ipv4RoutingTableEntry
ATPStaticRouting::GetRoute(uint32_t index) const
{
    NS_LOG_FUNCTION(this << index);
    uint32_t tmp = 0;
    for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++)
...
uint32_t
ATPStaticRouting::GetMetric(uint32_t index) const
{
    NS_LOG_FUNCTION(this << index);
    uint32_t tmp = 0;
    for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++)
...
void
ATPStaticRouting::RemoveRoute(uint32_t index)
{
    NS_LOG_FUNCTION(this << index);
    uint32_t tmp = 0;
    for (auto j = m_networkRoutes.begin(); j != m_networkRoutes.end(); j++)
...
Ptr<Ipv4Route>
ATPStaticRouting::RouteOutput(Ptr<Packet> p,
                              ...
                              const ErrorCallback& ecb)
{
    ...
    if (m_enableAggregation)
    {
        ...
        if (atpL4)
        {
            ...
            Ptr<Packet> forwardedPacket = atpL4->FilterPacket(packetCopy, ipHeader);
            // 继续正常的路由处理
            ...
            else
            {
                ...
                if (ipHeader.GetDestination().IsMulticast())
                {
                    ...
                    {
                        NS_LOG_LOGIC("Multicast local delivery to " << ipHeader.GetDestination());
                        lcb(p, ipHeader, iif); // 提交给 L4 (ATP/UDP)
                        return true;
                    }
                    else
                    {
        ...
        NS_LOG_INFO("Did not find ATP L4 protocol");
        return false;
    }
    else
    {
...
void
ATPStaticRouting::NotifyInterfaceUp(uint32_t i)
{
...
void
ATPStaticRouting::NotifyInterfaceDown(uint32_t i)
{
    NS_LOG_FUNCTION(this << i);
...
void
ATPStaticRouting::NotifyAddAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
...
void
ATPStaticRouting::NotifyRemoveAddress(uint32_t interface, Ipv4InterfaceAddress address)
{
    NS_LOG_FUNCTION(this << interface << " " << address.GetLocal());
    if (!m_ipv4->IsUp(interface))
    {
...
void
ATPStaticRouting::SetIpv4(Ptr<Ipv4> ipv4)
{
...
void
ATPStaticRouting::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    ...
    *os << "Node: " << m_ipv4->GetObject<Node>()->GetId() << ", Time: " << Now().As(unit)
        ...
        << ", ATPStaticRouting table" << std::endl;
