...
struct DmlJobRuntime {
    ...
    std::vector<Ptr<ATPSocket>> workerSockets; // 【新增】保存 worker 对应的 Socket
};
...
std::ofstream TxBytesStream_job1, TxBytesStream_job2;
uint64_t lastTimeJob1Bytes = 0, lastTimeJob2Bytes = 0;
uint64_t lastTimeJob1TxBytes = 0, lastTimeJob2TxBytes = 0;
uint8_t numJobs = 8;
...
MeasurementTx()
{
    ...
    uint64_t currentTxJob1 = 0;
    uint64_t currentTxJob2 = 0;

    // 获取 Job1 (索引 0) worker 发送的总和
    if (g_runtimeJobs.size() > 0) {
        currentTxJob1 += g_runtimeJobs[0].workerSockets[0]->GetTotalTxBytes();
    }
    // 获取 Job3 (索引 2) worker 发送的总和
    if (g_runtimeJobs.size() > 2) {
        currentTxJob2 += g_runtimeJobs[2].workerSockets[0]->GetTotalTxBytes();
    }

    uint64_t sentJob1Per100ms = currentTxJob1 - lastTimeJob1TxBytes;
    uint64_t sentJob2Per100ms = currentTxJob2 - lastTimeJob2TxBytes;

    TxBytesStream_job1 << now.GetMicroSeconds() << "\t" << currentTxJob1 << "\t"
                       << sentJob1Per100ms << std::endl;
    TxBytesStream_job2 << now.GetMicroSeconds() << "\t" << currentTxJob2 << "\t"
                       << sentJob2Per100ms << std::endl;

    lastTimeJob1TxBytes = currentTxJob1;
    lastTimeJob2TxBytes = currentTxJob2;

    Simulator::Schedule(MicroSeconds(100), &MeasurementTx);
}
...
int main(int argc, char* argv[]) {
    ...
    TxBytesStream_job1.open("atp-result/trace-spineleaf/job1-txBytes-trace.txt", std::ofstream::out | std::ofstream::trunc); // 【新增】
    TxBytesStream_job2.open("atp-result/trace-spineleaf/job3-txBytes-trace.txt", std::ofstream::out | std::ofstream::trunc); // 【新增】
    ...
    for (uint32_t spineIdx = 0; spineIdx < NetworkTopology::nSpines; spineIdx++) {
        for (uint32_t leafIdx = 0; leafIdx < NetworkTopology::nLeaves; leafIdx++) {
            ...
            if (spineDevice != nullptr) {
                spineDevice->SetThreshold(60); // 设置相应的队列包数阈值
    ...
    for (size_t jobIdx = 0; jobIdx < jobs.size(); jobIdx++) {
        ...
        for (const auto& workerName : job.workers) {
            ...
            g_runtimeJobs[jobIdx].workerSockets.push_back(worker_ATPSocket); // 【新增】
    ...
    if (sinkApps.size() >= 3)
    {
        ...
        Simulator::Schedule(Seconds(1.0), &MeasurementTx);  // 【新增】启动发送监控
    }
    ...
    TxBytesStream_job1.close(); // 【新增】
    TxBytesStream_job2.close(); // 【新增】
