...
Measurement(Ptr<ATPPacketSink> sink1, Ptr<ATPPacketSink> sink2)
{
    ...
    uint64_t currentTimeJob2Bytes = sink2->GetTotalRxJob(4);
...
int main(int argc, char* argv[]) {
    ...
    // 打开跟踪文件
    cwndStream_job1.open("atp-result/trace-stragglers/job1-cwnd-trace.txt", std::ofstream::out | std::ofstream::trunc);
    cwndStream_job2.open("atp-result/trace-stragglers/job4-cwnd-trace.txt", std::ofstream::out | std::ofstream::trunc);
    SinkBytesStream_job1.open("atp-result/trace-stragglers/job1-sinkBytes-trace.txt", std::ofstream::out | std::ofstream::trunc);
    SinkBytesStream_job2.open("atp-result/trace-stragglers/job4-sinkBytes-trace.txt", std::ofstream::out | std::ofstream::trunc);
    ...
    for (uint32_t spineIdx = 0; spineIdx < NetworkTopology::nSpines; spineIdx++) {
        for (uint32_t leafIdx = 0; leafIdx < NetworkTopology::nLeaves; leafIdx++) {
            ...
            if (spineDevice != nullptr) {
                spineDevice->SetThresholdBytes(120 * 300); // 设置相应的队列阈值
                spineDevice->SetEnableEcnBytes(true);
            }
        }
    }
    ...
    for (uint32_t leafIdx = 0; leafIdx < NetworkTopology::nLeaves; leafIdx++) {
        for (uint32_t serverLocalIdx = 0; serverLocalIdx < NetworkTopology::nServersPerLeaf; serverLocalIdx++) {
            ...
            if (leafDevice != nullptr) {
                leafDevice->SetThresholdBytes(65 * 300); // Leaf 交换机端口阈值 (参考论文 100Gbps 推荐值)
                leafDevice->SetEnableEcnBytes(true);
            }
        }
    }
    ...
    // 全网全局泊松调度背景流量 (Global Poisson All-to-All Workload)
    NS_LOG_INFO("Configuring Poisson all-to-all background traffic.");

    uint16_t bgPortBase = 5000;
    ApplicationContainer bgSinks;

    // 1. 在每个 Server 上安装 UDP 接收端
    for (uint32_t i = 0; i < NetworkTopology::nTotalServers; i++)
    {
    ...
    expRand->SetAttribute("Mean", DoubleValue(0.0002));
    ...
    double currentStartTime = 1.0; // 与主任务同时开始
    uint32_t maxGlobalFlows = 400000;
    uint32_t globalFlowCount = 0;

    while (currentStartTime < stopTime.GetSeconds() && globalFlowCount < maxGlobalFlows)
    {
        // 随机选择通信的 Host Pair (Source 和 Destination)
        uint32_t srcId = hostRand->GetInteger(0, NetworkTopology::nTotalServers - 1);
        uint32_t dstId = hostRand->GetInteger(1, NetworkTopology::nTotalServers - 1);
        dstId = (dstId + srcId) % NetworkTopology::nTotalServers;

        // 解析目标 IP (对应 leafServerInterfaces 的网卡 1)
        uint32_t leafIdx = getServerLeafIndex(NetworkTopology::serverStartIdx + dstId);
        uint32_t localIdx = getServerLocalIndex(NetworkTopology::serverStartIdx + dstId);
        Ipv4Address dstIp = leafServerInterfaces[leafIdx][localIdx].GetAddress(1);

        // 重尾分布: 80% 老鼠流 (50KB), 20% 大象流 (1MB)
        double prob = sizeProbRand->GetValue();
        uint32_t flowBytes = (prob < 0.8) ? 50000 : 1000000;
        ...
        // 将这个流挂载到随机选中的 srcId 上，并在 currentStartTime 启动
    ...
    if (sinkApps.size() >= 4)
    {
        Simulator::Schedule(Seconds(1.0), &Measurement, sinkApps[0], sinkApps[3]);
    }
