...
main(int argc, char* argv[])
{
    ...
    cwndStream_job1.open("atp-result/trace-sl-twojobs/job1-cwnd.txt",
                         std::ofstream::out | std::ofstream::trunc);
    cwndStream_job2.open("atp-result/trace-sl-twojobs/job2-cwnd.txt",
                         std::ofstream::out | std::ofstream::trunc);
    sinkBytesStream_job1.open("atp-result/trace-sl-twojobs/job1-sinkBytes.txt",
                              std::ofstream::out | std::ofstream::trunc);
    sinkBytesStream_job2.open("atp-result/trace-sl-twojobs/job2-sinkBytes.txt",
                              std::ofstream::out | std::ofstream::trunc);
    aggOccStream.open("atp-result/trace-sl-twojobs/s0-agg-occupancy.txt",
                      std::ofstream::out | std::ofstream::trunc);
    ...
    std::string bgDataRate = "80Gbps";
    bool hasJob1BackgroundFlow = (job1StragglerId == 0);
    uint32_t stragglerWaitingThresholdUs = 20;
    uint32_t stragglerSampleThreshold = 20;

    CommandLine cmd;
    cmd.AddValue("hasJob1BackgroundFlow", "Enable background flow on job1 straggler w0", hasJob1BackgroundFlow);
    cmd.AddValue("stragglerWaitingThresholdUs",
                 "Ignore straggler samples whose aggregator waiting time is at or below this value in microseconds",
                 stragglerWaitingThresholdUs);
    cmd.AddValue("stragglerSampleThreshold",
                 "Number of counted straggler samples required before applying straggler congestion logic",
                 stragglerSampleThreshold);
    cmd.Parse(argc, argv);
    job1StragglerId = hasJob1BackgroundFlow ? 0 : -1;
    ...
    std::cout << "stragglerWaitingThresholdUs: " << stragglerWaitingThresholdUs << std::endl;
    std::cout << "stragglerSampleThreshold: " << stragglerSampleThreshold << std::endl;
    ...
    std::vector<Ptr<ATPSocket>> workerSockets = {
        w0Socket, w1Socket, w2Socket, w3Socket, w4Socket, w5Socket, w6Socket, w7Socket};
    for (auto socket : workerSockets)
    {
        socket->GetTxBuffer()->SetStragglerWaitingThresholdUs(stragglerWaitingThresholdUs);
        socket->GetTxBuffer()->SetStragglerSampleThreshold(stragglerSampleThreshold);
    }
