...
NS_LOG_COMPONENT_DEFINE("ATP-Bursty-Straggler-Robust");
...
main(int argc, char* argv[])
{
    ...
    std::string traceDir = "stp-result/trace-bursty-robust/current";

    CommandLine cmd(__FILE__);
    cmd.AddValue("seed", "Random simulation seed", seed);
    cmd.AddValue("stragglerWaitingThresholdUs",
                 "Waiting-time threshold in microseconds for straggler detection",
                 stragglerWaitingThresholdUs);
    cmd.AddValue("stragglerSampleThreshold",
                 "Sample threshold for straggler detection",
                 stragglerSampleThreshold);
    cmd.AddValue("traceDir", "Directory for trace output files", traceDir);
    cmd.Parse(argc, argv);

    cwndStream_job1.open(traceDir + "/job1-cwnd.txt", std::ofstream::out | std::ofstream::trunc);
    cwndStream_job2.open(traceDir + "/job2-cwnd.txt", std::ofstream::out | std::ofstream::trunc);
    sinkBytesStream_job1.open(traceDir + "/job1-sinkBytes.txt", std::ofstream::out | std::ofstream::trunc);
    sinkBytesStream_job2.open(traceDir + "/job2-sinkBytes.txt", std::ofstream::out | std::ofstream::trunc);
    aggOccStream.open(traceDir + "/s0-agg-occupancy.txt", std::ofstream::out | std::ofstream::trunc);

    std::cout << "background: bursty" << std::endl;
    ...
    std::cout << "traceDir: " << traceDir << std::endl;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        sock->GetTxBuffer()->SetStragglerWaitingThresholdUs(stragglerWaitingThresholdUs);
        sock->GetTxBuffer()->SetStragglerSampleThreshold(stragglerSampleThreshold);
